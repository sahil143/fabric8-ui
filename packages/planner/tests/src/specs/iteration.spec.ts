...
describe('Iteration test', () => {
  ...
  it('should create a new child iteration', async () => {
    ...
    let expectedNewIteration = '/new Iteration1';
    ...
    expect(await planner.sidePanel.getIterationList()).toContain(expectedNewIteration);
  });

  it('updating iteration should update workitem associated to iteration', async () => {
    let dropdownIteration1 = 'newIteration',
      expecteddropdownIteration1 = '/newIteration',
    ...
    expect(await planner.workItemList.iterationText(workItemTitle1)).toBe(
      expecteddropdownIteration1,
    );
